...
class AccountsRegisterViewTestCase(TestCase):
    ...
    def register_post(self, email):
        with mock.patch(
            "worker.email.tasks.send_activation_email.delay",
...
class AccountsRegisterActivateViewTestCase(TestCase):
    ...
    def test_register_activate_get_success(self):
        with mock.patch(
            "worker.email.tasks.send_welcome_message.delay",
            self.mock_send_welcome_message,
        ):
            user = get_user_model().objects.create_user(
                email="test_user@example.com", password="test_password", is_active=False
            )
...
class AccountsLoginViewTestCase(TestCase):
    ...
    def test_login_success(self):
        user = get_user_model().objects.create_user(
            email="test_login_email@example.com", password="password", is_active=True
        )

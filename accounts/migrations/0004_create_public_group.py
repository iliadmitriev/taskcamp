...
def create_public_group(apps, schema_editor):
    ...
    groups = apps.get_model("auth", "Group").objects.using(
        schema_editor.connection.alias
    )
    permissions = apps.get_model("auth", "Permission").objects.using(
        schema_editor.connection.alias
    )
    users = apps.get_model("accounts", "User").objects.using(
        schema_editor.connection.alias
    )
    group_public, _ = groups.get_or_create(name="public")
    perms = permissions.all()
    perms_public = perms.exclude(
    ...
    group_public.permissions.add(
        permissions.get(content_type__app_label="projects", codename="add_comment")
    )
...
def delete_public_group(apps, schema_editor):
    ...
    try:
        group_public = Group.objects.using(schema_editor.connection.alias).get(
            name="public"
        )
        group_public.delete()
...
class Migration(migrations.Migration):
    ...
    operations = [migrations.RunPython(create_public_group, delete_public_group)]

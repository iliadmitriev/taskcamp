...
from projects.admin import CommentAdmin, ProjectAdmin, TaskAdmin
...
class ProjectAdminTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.comment = Comment.objects.create(
            description="test comment", task=self.task1
        )
    ...
    def test_task_admin_view_on_site_url(self):
        ...
        self.assertEqual(
            url, reverse("projects-task-detail", kwargs={"pk": self.task1.id})
        )

    def test_comment_admin_view_on_site_url(self):
        url = self.comment_admin.get_view_on_site_url(obj=self.comment)
        self.assertEqual(
            url, reverse("projects-task-detail", kwargs={"pk": self.comment.task.id})
        )
